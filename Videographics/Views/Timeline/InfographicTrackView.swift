...
struct InfographicTrackView: View {
    let layer: InfographicLayer
        ...
        GeometryReader { geometry in
            ZStack(alignment: .leading) {
                ...
                ForEach(layer.clips) { clip in
                    infographicClipView(for: clip, in: geometry)
                }
    ...
    @ViewBuilder
    private func infographicClipView(for clip: InfographicClip, in geometry: GeometryProxy) -> some View {
        let isBeingDragged = isDragging && draggingClipId == clip.id
        let isBeingTrimmed = isTrimming && trimmingClipId == clip.id
        let isSelected = viewModel.editingInfographicClip?.id == clip.id
        ...
        ZStack {
            ...
            HStack(spacing: 0) {
                // Leading trim handle
                Rectangle()
                    .fill(isBeingTrimmed && trimEdge == .leading ? Color.yellow : Color.orange)
                    .frame(width: 6)
                ...
                Rectangle()
                    .fill(isBeingTrimmed && trimEdge == .trailing ? Color.yellow : Color.orange)
                    .frame(width: 6)
                    .cornerRadius(4, corners: [.topRight, .bottomRight])
            }

            // Chart type icon and title
            HStack(spacing: 4) {
                Image(systemName: clip.chartType.iconName)
                    .font(.caption2)
                    .foregroundColor(.white)

                if let title = clip.titleText {
                    Text(title)
                        .font(.caption2)
                        .foregroundColor(.white)
                        .lineLimit(1)
                } else {
                    Text(clip.chartType.displayName)
                        .font(.caption2)
                        .foregroundColor(.white)
                        .lineLimit(1)
                }
        ...
        .onTapGesture {
            viewModel.editInfographicClip(clip)
        }
        .gesture(
    ...
    private func dragGesture(for clip: InfographicClip) -> some Gesture {
        DragGesture()
            .onChanged { value in
                if !isDragging {
    ...
    private func trimGesture(for clip: InfographicClip, in geometry: GeometryProxy) -> some Gesture {
...
#Preview {
    InfographicTrackView(
        layer: InfographicLayer(name: "Infographic", zIndex: 102),
